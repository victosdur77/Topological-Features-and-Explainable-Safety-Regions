...
for i, run in tqdm(exp.runs.items(), desc="Procesando runs"): # in data.items()
    ...
    deadlocks.append(count_deadlocks(run.deadlocks,run.maximal_steps*run.time_step))
...
print(len(sms),len(etalist),len(taulist),len(collisions),len(deadlocks),len(meanEntropy),len(medianEntropy),len(stdsEntropy),len(iqrsEntropy))
dataset = pd.DataFrame(zip(sms,etalist,taulist,collisions,deadlocks,meanEntropy,medianEntropy,stdsEntropy,iqrsEntropy),columns = ["SafetyMargin","Eta","Tau","NumberOfCollisions","NumberOfDeadlocks","meanEntropy","medianEntropy","stdsEntropy","iqrsEntropy"])
print(dataset.head())
